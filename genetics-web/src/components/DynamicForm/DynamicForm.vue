  ...
  >
    ...
    >
      ...
      >
        <el-form-item
          :label="cell.label"
          :prop="cell.controlKey"
          :rules="buildRules(cell.controlId)"
        >
          <control-renderer
            :cell="cell"
          ...
          <!-- Tips 提示 -->
          <div
            v-if="controlMap[cell.controlId]?.tips"
            class="control-tips"
          >
...
// 根据控件配置构建 Element Plus 校验规则
function buildRules(controlId) {
  const control = controlMap.value[controlId]
  if (!control) return []
  const rules = []
  if (control.required) {
    rules.push({ required: true, message: `${control.controlName || '该字段'}不能为空`, trigger: 'blur' })
  }
  if (control.minLength) {
    rules.push({ min: control.minLength, message: `最少${control.minLength}个字符`, trigger: 'blur' })
  }
  if (control.maxLength) {
    rules.push({ max: control.maxLength, message: `最多${control.maxLength}个字符`, trigger: 'blur' })
  }
