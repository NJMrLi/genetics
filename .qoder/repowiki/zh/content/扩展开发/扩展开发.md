...
## 简介
本扩展开发文档面向系统扩展者，围绕VAT&EPR动态表单系统，提供自定义控件开发、业务实体扩展、插件系统设计、控件注册与渲染器实现、验证规则配置、新业务实体集成、FormDataConverter扩展与数据转换逻辑、文件上传服务集成、第三方系统对接以及API扩展开发的完整框架与实现指导。同时明确扩展开发的限制条件与最佳实践，帮助快速、安全地完成二次开发与定制化集成。
...
graph TB
subgraph "后端"
A["控制器层<br/>FormControlController/FormTemplateController/FormInstanceController/ServiceCategoryController"]
B["服务层<br/>FormControlService/FormTemplateService/FormInstanceService"]
C["映射层<br/>FormControlMapper/FormTemplateMapper/FormInstanceMapper"]
D["实体与领域模型<br/>FormControl/FormTemplate/FormInstance<br/>domain/Company/CompanyLegalPerson"]
E["转换器<br/>FormDataConverter"]
F["通用返回封装<br/>Result"]
end
subgraph "前端"
G["API层<br/>formControl.js/formTemplate.js/formInstance.js/serviceCategory.js"]
H["视图层<br/>ControlList/TemplateList/TemplateDesigner/InstanceList/InstanceForm"]
...
BE->>Biz : "调用FormDataConverter.convert(formData)"
Biz-->>BE : "返回Map<className, 实体>"
BE->>DB : "更新状态=已提交"
BE-->>FE : "返回转换结果"
```
...
## 结论
本扩展开发文档提供了从控件到实体、从渲染到转换、从前端到后端的全链路扩展指南。通过标准化的control_key命名、清晰的注册机制与插件化的渲染器设计，系统具备良好的可扩展性与可维护性。建议在实际落地中优先采用注解扫描替代静态注册，完善错误处理与日志体系，并结合缓存与异步策略提升整体性能与稳定性。
