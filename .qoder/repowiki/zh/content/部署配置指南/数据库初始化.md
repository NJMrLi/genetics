...
## 更新摘要
**变更内容**
- 更新了form_instance表的字段定义，增加了order_status_id、service_start_time、service_end_time等业务状态字段
- 修正了数据库初始化脚本中的字段注释和约束配置
- 更新了JSON字段的使用说明和数据类型映射
- 完善了数据库连接配置和字符集设置说明
...
**更新** 基于最新的数据库初始化脚本，系统现已支持更完善的业务状态管理和服务流程跟踪。
...
**更新** 数据库初始化脚本现已包含完整的表结构定义和约束配置，确保数据一致性和完整性。
...
**章节来源**
- [init.sql: 5-80:5-80](file://genetics-server/src/main/resources/db/init.sql#L5-L80)
...
**图表来源**
- [init.sql: 9-31:9-31](file://genetics-server/src/main/resources/db/init.sql#L9-L31)
...
**章节来源**
- [init.sql: 9-31:9-31](file://genetics-server/src/main/resources/db/init.sql#L9-L31)
- [FormControl.java: 48-58:48-58](file://genetics-server/src/main/java/com/genetics/entity/FormControl.java#L48-L58)
...
**图表来源**
- [init.sql: 34-49:34-49](file://genetics-server/src/main/resources/db/init.sql#L34-L49)
...
| status | TINYINT(1) | NOT NULL DEFAULT 0 | 状态: 0草稿 1发布 |
...
**章节来源**
- [init.sql: 34-49:34-49](file://genetics-server/src/main/resources/db/init.sql#L34-L49)
- [FormTemplate.java: 36-40:36-40](file://genetics-server/src/main/java/com/genetics/entity/FormTemplate.java#L36-L40)
...
**图表来源**
- [init.sql: 51-72:51-72](file://genetics-server/src/main/resources/db/init.sql#L51-L72)
...
| order_status_id | INT | NOT NULL DEFAULT 10 | 业务状态ID: 10待提交 20待审核 30待递交 31组织处理 32税局处理 33当地同事处理 40已完成 50已驳回 99已终止 |
| service_start_time | DATETIME | DEFAULT NULL | 服务开始时间 |
| service_end_time | DATETIME | DEFAULT NULL | 服务结束时间 |
...
**更新** 新增了order_status_id字段，用于支持完整的业务流程状态管理，包括服务开始时间和结束时间的跟踪。

**章节来源**
- [init.sql: 51-72:51-72](file://genetics-server/src/main/resources/db/init.sql#L51-L72)
- [FormInstance.java: 39-52:39-52](file://genetics-server/src/main/java/com/genetics/entity/FormInstance.java#L39-L52)
...
**图表来源**
- [init.sql: 9-72:9-72](file://genetics-server/src/main/resources/db/init.sql#L9-L72)
...
**更新** form_instance表新增了template_id索引，用于优化按模板ID的查询性能。
...
**章节来源**
- [application.yml: 8-11:8-11](file://genetics-server/src/main/resources/application.yml#L8-L11)
...
**更新** 最新的数据库初始化脚本包含了完整的表结构定义和约束配置，确保了数据的一致性和完整性。新增的业务状态管理字段支持了更完整的业务流程跟踪，提高了系统的实用性。
...
**更新** 基于application.yml中的配置，系统使用以下数据库连接参数：

- **数据库URL**: jdbc:mysql://localhost:3306/genetics_db?useUnicode=true&characterEncoding=utf8&useSSL=false&serverTimezone=Asia/Shanghai
- **用户名**: root
- **密码**: 123456
...
**章节来源**
- [application.yml: 8-11:8-11](file://genetics-server/src/main/resources/application.yml#L8-L11)
- [init.sql: 5](file://genetics-server/src/main/resources/db/init.sql#L5)